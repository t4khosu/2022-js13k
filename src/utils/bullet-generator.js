import {collides, degToRad, PoolClass, radToDeg} from "kontra";
import {Bullet} from "../entities/bullet-pool";
...
export class BulletGeneratorConfig {
...
export class Beam extends PoolClass {
    maxSize = 5000
    tick = 0
    rotation = 10
    ...
    get() {
        this.tick += 1
        if (this.tick % this.config.fireRate === 0) {
            let dx = this.config.dx
            let dy = this.config.dy
            if (this.config.dxf)
                dx = this.config.dxf(this.tick)
            if (this.config.dyf)
                dy = this.config.dyf(this.tick)

            super.get({...this.config, dx: dx, dy: dy})
...
export class BulletGenerator {
    ...
    generate() {
        const beams = []
        let angle = 0
        for (let i = 0; i < this.config.beams; i++) {
            const startAngle = angle
            const dxf = (tick) => {
                let o = startAngle
                if (this.config.spinRate) {
                    const maxAngle = this.config.inverseSpin === 0 ? 360 : 2 * this.config.inverseSpin
                    let tickSpin = tick * this.config.spinRate % maxAngle
                    if (this.config.inverseSpin && tickSpin > this.config.inverseSpin) {
                        tickSpin = (this.config.inverseSpin - (tickSpin % this.config.inverseSpin))
                    }
                    o = o + tickSpin
                }
                return Math.cos(degToRad(o))
            }
            // negative coordinate because system is y-reversed
            const dyf = (tick) => {
                let o = startAngle
                if (this.config.spinRate) {
                    const maxAngle = this.config.inverseSpin === 0 ? 360 : 2 * this.config.inverseSpin
            ...
            console.log(bConf)
            angle += this.config.beamSpread
            const beam = new Beam(bConf)
        ...
        // return () => {
        //     // apply field
        //     let totalAccelerationX = 1;
        //     let totalAccelerationY = 1;
        //
        //
